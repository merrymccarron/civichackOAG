#!/usr/bin/python
# -*- coding: utf-8 -*-
## Documentation with great explanation: http://open-city.github.io/dedupe/doc/mysql_example.html
"""
...
# ## Setup
MYSQL_CNF = os.path.abspath('.') + '/my.cnf'
...
# print('connected to mysql, exiting now')
# close_connection()
...
DONOR_SELECT = "SELECT CORP, LAST_NAME, FIRST_NAME, ZIP, STATE, ADDR, " \
    "CITY, AMOUNT, DATE1, FILER_ID, CHECK_NO, RECIPIENT, BOEE_ID from raw_table"
...
if os.path.exists(settings_file):
    ...
    # account
    fields = [{'field' : 'CORP', 'variable name' : 'CORP',
             'type': 'String', 'has missing' : True},

            {'field' : 'LAST_NAME', 'variable name' : 'LAST_NAME',
            'type': 'String', 'has missing' : True},
            
            {'field' : 'FIRST_NAME', 'variable name' : 'FIRST_NAME',
             'type': 'String', 'has missing' : True},

            {'field' : 'ADDR', 'variable name' : 'ADDR',
             'type' : 'String'},
            ...
            {'type' : 'Interaction',
             'interaction variables' : ['CORP', 'ADDR']},
            
            {'type' : 'Interaction', 
             'interaction variables' : ['LAST_NAME', 'ADDR']}
            ]
...
c.execute("CREATE TABLE blocking_map "
        "(block_key VARCHAR(200), BOEE_ID INTEGER) "
...
for field in deduper.blocker.index_fields :
    c2.execute("SELECT DISTINCT %s FROM raw_table" % field)
    field_data = [row[0] for row in c2 if row[0] != None]
    # print(type(field_data[0]))
    # print(field_data[0])
    deduper.blocker.index(field_data, field)

# Now we are ready to write our blocking map table by creating a
# generator that yields unique `(block_key, BOEE_ID)` tuples.
print('writing blocking map')

c.execute(DONOR_SELECT)
full_data = ((row['BOEE_ID'], row) for row in c)
...
c.execute("ALTER TABLE blocking_map "
        "ADD UNIQUE INDEX (block_key, BOEE_ID)")
...
c.execute("CREATE TABLE plural_key "
        ...
        " (SELECT block_key, "
        "  GROUP_CONCAT(BOEE_ID ORDER BY BOEE_ID) AS block "
        "  FROM blocking_map "
...
c.execute("CREATE TABLE plural_block "
        "(SELECT block_id, BOEE_ID "
        " FROM blocking_map INNER JOIN plural_key "
        " USING (block_key))")

logging.info("adding BOEE_ID index and sorting index")
c.execute("ALTER TABLE plural_block "
        "ADD INDEX (BOEE_ID), "
        "ADD UNIQUE INDEX (block_id, BOEE_ID)")
...
c.execute("CREATE UNIQUE INDEX BOEE_IDx ON covered_blocks (BOEE_ID)")
...
def candidates_gen(result_set) :
    ...
    for row in result_set :
        ...
        records.append((row['BOEE_ID'], row, smaller_ids))

    if records :
        yield records

c.execute("SELECT BOEE_ID, FIRST_NAME, LAST_NAME, CORP, CITY"
        "ZIP, STATE, ADDR, AMOUNT, DATE1, FILER_ID, CHECK_NO"
        "RECIPIENT, block_id, smaller_ids "
        "FROM smaller_coverage "
        "INNER JOIN raw_table "
        "USING (BOEE_ID) "
        "ORDER BY (block_id)")

print('clustering...')
clustered_dupes = deduper.matchBlocks(candidates_gen(c), threshold=0.5)
...
c.execute("CREATE TABLE entity_map "
        "(BOEE_ID INTEGER, canon_id INTEGER, "
        " cluster_score FLOAT, PRIMARY KEY(BOEE_ID))")

for cluster, scores in clustered_dupes :
    cluster_id = cluster[0]
    for BOEE_ID, score in zip(cluster, scores) :
        c.execute('INSERT INTO entity_map VALUES (%s, %s, %s)',
            (BOEE_ID, cluster_id, score))
...
c.execute("CREATE TEMPORARY TABLE e_map "
        "SELECT IFNULL(canon_id, BOEE_ID) AS canon_id, BOEE_ID "
        "FROM entity_map "
        "RIGHT JOIN donors USING(BOEE_ID)")


c.execute("SELECT CONCAT_WS(' ', raw_table.FIRST_NAME, raw_table.LAST_NAME) AS NAME, "
        ...
        " DESC LIMIT 10) "
        "AS donation_totals "
        "WHERE donors.BOEE_ID = donation_totals.canon_id")
...
c.execute("SELECT CONCAT_WS(' ', raw_table.FIRST_NAME, raw_table.LAST_NAME) as NAME, "
        ...
        "FROM raw_table INNER JOIN contributions "
        "USING (BOEE_ID) "
        "GROUP BY (BOEE_ID) "
